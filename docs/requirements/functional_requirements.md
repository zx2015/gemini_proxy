...
> - 2026-07-07：v0.1.0-r4 新增 §2.9 关键路径调试日志功能，定义文件轮转、配置开关与记录阶段。
...
### 2.9 关键路径调试日志 (Key Path Debug Logging)

为了排查类似 `gemini-cli` 等客户端与本网关之间的不稳定连接，需要实现专门的、独立于普通 stdout 日志的关键路径调试日志功能。

#### 2.9.1 日志存储与轮转要求 (HDD)
- **独立写入**：所有的详细调试日志仅写入到指定的物理文件（HDD 中），不污染 stdout，以防容器日志大小爆炸。
- **文件大小限制与备份**：单个文件最大支持 `20MB`。最大保存 2 个历史备份文件（连同主文件共 3 个文件循环轮转：`debug.log`, `debug.log.1`, `debug.log.2`），避免磁盘耗尽。
- **配置开关**：增加全局开关变量，可控制启用或关闭此调试日志。默认关闭以保障零磁盘 I/O 损耗。

#### 2.9.2 详细追踪节点
调试日志应包含以下四个阶段的详细信息，且每次调用应被分配一个唯一的 Request ID（如 `req_xxxx`）以便在并发请求时通过 `grep` 过滤：
1. **Gemini 原始入站请求**：包括请求 Path、Query 参数（如有敏感 API Key 需脱敏）、请求体 Body 全文。
2. **OpenAI 转换出站请求**：包括转给上游的 model 字段、完整的 `messages` 数组（含 `tool_calls`）等。
3. **上游响应**：
   - 非流式：上游返回的完整 Response 文本。
   - 流式：上游下发的每一帧原始 SSE `data: {...}` 内容。
4. **最终回传客户端数据**：
   - 非流式：最终的 Gemini JSON 响应内容。
   - 流式：网关最终 `yield` 发送回客户端的每一个 JSON 分片（符合 JSON 数组流形式的字节流）。
...
- **稳定性**：上游 429 / 500 / 502 / 503 / 504 必须能被识别并转换为合规 the Gemini 错误响应，**严禁**将上游原始 HTML / 堆栈泄漏给客户端。
...
| `DEBUG_LOG_ENABLED` | 否 | `False` | 是否启用详细的关键路径调试日志（开关） |
| `DEBUG_LOG_PATH` | 否 | `/app/logs/debug.log` | 调试日志在物理 HDD 上（容器内路径）的存储地址 |
